...
def pack_json(sender_pk, receive_pk, capsule, kfrag) -> dict[str, str]:
    data = {}
...
def get_test_input():
    ...
    kfrags = umbral.generate_kfrags(
        delegating_sk=sender_private_key,
...
def test_simple():
    ...
    r = requests.get(
        f"{proxy_base_url}get/kfrag/{bytes(sender_public_key).hex()}/{bytes(receiver_public_key).hex()}"
    )
...
def test_get_without_put():
    ...
    r = requests.get(
        f"{proxy_base_url}get/kfrag/{data['sender_pk']}/{data['receiver_pk']}"
    )
...
def test_missing_get_fields():
    ...
    r = requests.get(f"{proxy_base_url}get/kfrag/{data['sender_pk']}")
